...
while True:
    ...
    if 'currency_pair.txt' in listdir():
        ...
        value = file_to_read_from.read()
        file_to_read_from.close()
        remove('currency_pair.txt')
        temp_contract = Forex(value)
        ...
        bars      = ib.reqHistoricalData(
            temp_contract, # <<- pass in your contract object here
    ...
    if 'trade_order.p' in listdir():
        ...
        print(trd_ordr['trade_currency'])
        new_contract = Forex(trd_ordr['trade_currency'])
        mk_order = MarketOrder(trd_ordr['action'],trd_ordr['trade_amt'])
        mk_order.account = acc_number
        new_order = ib_orders.placeOrder(new_contract,mk_order)
